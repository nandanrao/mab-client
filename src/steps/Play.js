...
export default (props) => {
  ...
  const allBoxesClicked = getResults(boxes).filter(res => !!res).reduce((a,b) => a+1, 0) === boxes.length
  ...
  return <div className="play">
    <Rounds boxes={boxes} />
    {winnings > 0 ? null : <div className="boxes"> {playboxes[curr] || playboxes[curr-1]} </div>}
